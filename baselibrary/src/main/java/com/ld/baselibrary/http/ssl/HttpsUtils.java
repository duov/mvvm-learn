...
public class HttpsUtils {
     ...
     * 用含有服务端公钥的证书校验服务端证书
     */
    public static SSLParams getSslSocketFactory(String... certificatesStr) {
        InputStream[] is = new InputStream[certificatesStr.length - 1];
        int index = 0;
        for (String certStreamStr : certificatesStr) {
            is[index] = new ByteArrayInputStream(certStreamStr.getBytes());
            index++;
        }
        return getSslSocketFactoryBase(null, null, null, is);
    }
